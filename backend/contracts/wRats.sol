 ...
 * Users can transfer native coins, tokens, and Non-fungible tokens to a designated recipient stealth address,
...
contract wRats {
    ...
    // where sharedSecret is 1 byte of stealth address prefixed with ephemeral key , used for verification
    // ephemeral public keys = sharedSecret + (x_cor + y_cor) sharedSecret + (ephemeral keys)
    ...
    // @notice Define an array to store the logs of published ephemeral public keys
    ...
    event publicKeys(bytes32 x_cor, bytes32 y_cor, bytes1 sharedSecret);
    ...
    // @param sharedSecret: 1-bytes of stealth address prefixed with ephemeral key
